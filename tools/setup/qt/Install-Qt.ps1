...
#               : Copyright (C) 2020 - 2025 HAMLIB SDK Contributors
# License ......: GPLv3
#
...
function InstallQt($script) {
	...
	IF ($exitCode -eq 1) {
		Write-Host "  --> Install Check Passed ($exitCode)" -ForegroundColor Yellow
	} else {
...
function UpdateQt {
	...
	if (Test-Path $exe) {
		...
		$msg="*** *** Cannot locate Qt Installer *** ***"
		InstallError($msg)
	}
...
#	Write-Host "   InstallQt.ps1 manual .. Install minimal components"  # Deprecade
#	Write-Host "   InstallQt.ps1 min ..... Install minimal components" 	# Deprecated
	Write-Host "   InstallQt.ps1 full .... Install all components for Qt $env:QT6_VER"
...
Write-Host "------------------------------------------------------"
Write-Host " JTSDK64 Tools Qt Deployment"
Write-Host "------------------------------------------------------"
...
Write-Host ""
Write-Host "  full   - Scripted installation of Qt $env:QT6_VER"
Write-Host "  manual - Manual deployment of Qt"
